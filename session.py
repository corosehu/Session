...
# HTML Template (Cyberpunk + File Manager + Live Logs)
HTML_CONTENT = """
<!DOCTYPE html>
        ...
        function appData() {
            return {
                ...
                async submitLogin() {
                    ...
                    this.addLog("Connecting to Instagram APIs...");
                    ...
                    try {
                        ...
                        if (response.ok) {
                            ...
                            // Show the detailed error in the terminal
                            const errorDetail = data.detail || 'Unknown error';
                            const rawError = data.raw_error || '';
                            this.addLog(`FAILED: ${errorDetail}`, "error");
                            if (rawError) {
                                this.addLog(`RAW: ${rawError}`, "error");
                            }
                        }
                    } catch (e) {
                ...
                async submitVerification() {
                    ...
                    try {
                        ...
                        if (response.ok) {
                            ...
                            this.addLog(`VERIFICATION FAILED: ${data.detail}`, "error");
                        }
                    } catch (e) {
                         this.addLog(`NETWORK ERROR: ${e.message}`, "error");
                    } finally {
...
async def api_login(
    ...
    try:
        ...
        error_msg = str(e)
        if "challenge_required" in error_msg or "checkpoint_required" in error_msg:
             return JSONResponse(status_code=403, content={
                 "detail": "Checkpoint Required: Please open Instagram on your mobile and approve the login attempt.",
                 "raw_error": error_msg
             })
        return JSONResponse(status_code=403, content={"detail": f"Connection Error: {e}", "raw_error": error_msg})

    except instaloader.BadCredentialsException as e:
        return JSONResponse(status_code=401, content={"detail": "Invalid password or username.", "raw_error": str(e)})
        ...
        return JSONResponse(status_code=500, content={"detail": f"System Error: {error_msg}", "raw_error": error_msg})
...
async def verify_2fa(
    ...
    try:
        ...
        return JSONResponse(status_code=400, content={"detail": "Invalid code.", "raw_error": str(e)})
