...
def api_bot_icon():
    """Proxy bot icon to avoid exposing token in HTML."""
    try:
        bot_info = bot.get_me()
    ...
    return RedirectResponse("https://cdn-icons-png.flaticon.com/512/2111/2111463.png")

@app.get("/", response_class=HTMLResponse)
def index():
    icon_url = "/api/bot-icon"
...
def install_cloudflared():
    """Detects OS/Arch and downloads cloudflared binary."""
...
def start_cloudflared_tunnel():
    ...
    try:
        ...
        while time.time() - start_t < 60:
            # We check stderr for the URL
            line = process.stderr.readline()
            if line:
                # logger.info(f"[Cloudflare] {line.strip()}")
                match = re.search(r"(https://[a-zA-Z0-9-]+\.trycloudflare\.com)", line)
                if match:
                    WEBAPP_URL = match.group(1)
                    logger.info(f"✅ Cloudflare Tunnel established: {WEBAPP_URL}")
                    try:
...
def start_tunnel():
    ...
    # Priority 1: Cloudflare (Most robust HTTPS)
    if start_cloudflared_tunnel():
        return

    # Fallback: SSH Tunnels
    if not shutil.which("ssh"):
        logger.warning("SSH not found. Cannot start backup tunnels.")
        return
...
if __name__ == "__main__":
    ...
    # Start Telegram Bot (Main Thread) with conflict handling
    import time
    while True:
        try:
            ...
            if "Conflict" in str(e) or "409" in str(e):
                logger.warning("Bot conflict detected (another instance running). Retrying in 10s...")
                time.sleep(10)
            else:
