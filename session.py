...
def start_tunnel():
    ...
    logger.info("Starting localhost.run tunnel for HTTPS...")
    try:
        ...
        while time.time() - start_time < 30: # Wait up to 30 seconds
            ...
            if not line:
                break
            # Output format: "Connect to your tunnel at https://something.localhost.run"
            match = re.search(r"(https://[a-zA-Z0-9-]+\.localhost\.run)", line)
            if match:
                WEBAPP_URL = match.group(1)
                logger.info(f"✅ Tunnel established: {WEBAPP_URL}")
                return

        logger.error("Failed to capture tunnel URL within timeout.")
    except Exception as e:
