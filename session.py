    ...
    def login(self):
        ...
        data = {
            "signed_body": "SIGNATURE." + json.dumps({
                ...
                "password": str(self.password), # Plaintext password is often accepted in this signed_body format for this specific API version
                 # BUT the user code used enc_password. Let's try to stick to user's enc_password format if possible,
                 # or use the provided structure. User provided:
                 # "enc_password": f"#PWD_INSTAGRAM:0:{round(time.time(), 3)}:{self.password}"
                 # We will use the user's data payload structure.
            }),
             # Override with User's specific data structure
            "signed_body": "SIGNATURE." + json.dumps({
                 "_csrftoken": "missing",
        ...
        # The user's code had `data` keys like "signed_body=...". requests `data=` expects dict or string.
        # User code: data = { "signed_body=...": "missing", ... } which looks malformed or copy-paste error.
        # Let's clean it up to standard signature format or raw string.

        # CORRECTED DATA PAYLOAD BASED ON USER SNIPPET INTENT:
        # The user snippet had keys like "signed_body=SIGNATURE._csrftoken": "missing".
        # This implies they are sending a form where the key IS the signed body string?
        # No, typically it is `signed_body` = `SIGNATURE.payload`.
        # Let's implement the standard way this API works while respecting the headers.
        ...
        try:
            ...
            if "challenge" in resp_json:
                 return {"status": "challenge_required", "message": "Checkpoint challenge required. Please verify on device."}

            if "message" in resp_json:
                return {"status": "fail", "message": resp_json["message"]}
...
async def api_login(
    ...
    try:
        ...
        if result["status"] == "success":
             ...
             return JSONResponse(status_code=403, content={"detail": result["message"]})
