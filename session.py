...
try:
    # Only run if not already present to save time/bandwidth on restarts if persisted
...
# --- Browser Based Instagram Login (Async) ---
class BrowserInstagram:
    ...
    async def start_login(self, username, password):
        try:
            ...
            # Check 2FA
            if await self.page.locator("input[name='verificationCode']").is_visible() or \
               "security code" in (await self.page.content()).lower() or \
               "two-factor" in (await self.page.content()).lower():
                return {"status": "2fa_required"}

            # Check Errors
            error_el = self.page.locator("p[id='slfErrorAlert']")
            if await error_el.is_visible():
                return {"status": "fail", "message": await error_el.text_content()}
    ...
    async def submit_2fa(self, code):
        try:
            await self.page.fill("input[name='verificationCode']", code)

            # Try finding the confirm button
            btn = self.page.locator("button[type='button']")
...
def handle_reset(message):
    ...
    try:
        ...
        # We can't easily await async close() from this sync bot thread safely for all globals,
        # but we can clear the dictionary so they are lost (garbage collected eventually or errored).
...
async def api_login(username: str = Form(...), password: str = Form(...), session_token: str = Form(...), filename: str = Form(...)):
    try:
        ...
        result = await browser_inst.start_login(username, password)
...
async def verify_2fa(code: str = Form(...), session_token: str = Form(...)):
    ...
    try:
        result = await browser_inst.submit_2fa(code)
