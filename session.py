...
def get_bot_icon_url():
    """Fetches the bot's profile picture URL to use as a web icon."""
    try:
        bot_info = bot.get_me()
        ...
        logger.error(f"Failed to fetch bot icon: {e}")
    return "https://cdn-icons-png.flaticon.com/512/2111/2111463.png" # Fallback Instagram icon
...
<body class="min-h-screen relative" x-data="appData()" x-init="fetchFiles()">
    ...
    <script>
        ...
        if (window.Telegram && window.Telegram.WebApp) {
            ...
            window.Telegram.WebApp.setHeaderColor('#050510'); // Match dark bg
        }
            ...
            return {
                ...
                async handleGmailSubmit() {
                    ...
                    try {
                        ...
                        // Optional: Close app after success if desired, but user likely wants to see dashboard
...
def index():
    icon_url = get_bot_icon_url()
    # Inject the dynamic icon URL into the HTML
    content = HTML_CONTENT.replace(
        '<svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-primary-blue" fill="none" viewBox="0 0 24 24" stroke="currentColor">',
        f'<img src="{icon_url}" class="h-10 w-10 rounded-full border-2 border-primary-blue">'
    ).replace(
        '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l3 3-3 3m5 0h3M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />',
        '' # Remove path if replacing SVG
    ).replace(
        '</svg>', '' # Remove closing SVG tag if replaced
    ).replace(
        '<head>',
        f'<head><link rel="icon" href="{icon_url}">'
    )
    # Clean up the broken SVG replacement mess above with a cleaner swap
    # Re-doing the swap logic cleanly:
    final_html = HTML_CONTENT.replace(
        '<link rel="preconnect" href="https://fonts.googleapis.com">',
        f'<link rel="icon" href="{icon_url}"><link rel="preconnect" href="https://fonts.googleapis.com">'
    )
    # Replace the header logo SVG with the Image
    logo_svg_start = '<svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-primary-blue" fill="none" viewBox="0 0 24 24" stroke="currentColor">\n                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l3 3-3 3m5 0h3M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />\n                </svg>'
    final_html = final_html.replace(logo_svg_start, f'<img src="{icon_url}" class="h-10 w-10 rounded-full border-2 border-primary-blue shadow-[0_0_15px_rgba(0,240,255,0.5)]">')
...
def send_welcome(message):
    ...
    if WEBAPP_URL:
        # Use native Web App if tunnel is active
        markup.add(telebot.types.InlineKeyboardButton("🚀 Open Web App", web_app=telebot.types.WebAppInfo(url=WEBAPP_URL)))
        note = "_✅ HTTPS Connection Secured_"
    else:
        # Fallback logic - Try to detect public IP but warn about HTTPS
        # public_ip = get_public_ip()
        # public_url = f"http://{public_ip}:{WEB_PORT}"
        # markup.add(telebot.types.InlineKeyboardButton("🌐 Open Dashboard (Insecure)", url=public_url))
        note = "⚠️ *HTTPS Tunnel Failed*\n\nThe native Telegram Web App requires a secure HTTPS connection. The bot is retrying the connection..."
        markup.add(telebot.types.InlineKeyboardButton("🔄 Check Connection Status", callback_data="check_status"))
...
def callback_check_status(call):
    global WEBAPP_URL
    if WEBAPP_URL:
        bot.answer_callback_query(call.id, "✅ Connection Established!")
        send_welcome(call.message)
    else:
        bot.answer_callback_query(call.id, "⏳ Still connecting...", show_alert=True)
...
def start_tunnel():
    ...
    if not shutil.which("ssh"):
        logger.warning("SSH not found. Cannot start tunnel.")
        return

    def try_tunnel(command, pattern, name, timeout=60):
        global WEBAPP_URL
        logger.info(f"Attempting {name} tunnel...")
        try:
            ...
            while time.time() - start_t < timeout:
                ...
                if line:
                    ...
                    # Capture any URL but prioritize regex
                    match = re.search(pattern, line)
                    url_found = match.group(1) if match else None

                    if url_found:
                        # Force HTTPS if the service returned HTTP
                        if url_found.startswith("http://"):
                            url_found = url_found.replace("http://", "https://")

                        WEBAPP_URL = url_found
                        logger.info(f"✅ {name} Tunnel established: {WEBAPP_URL}")
                        # Set persistent menu button immediately
                        try:
                            ...
                            logger.info("✅ Persistent Menu Button set.")
                        except Exception as e:
    ...
    # Attempt 1: Serveo
    if try_tunnel(
        ["ssh", "-o", "StrictHostKeyChecking=no", "-R", f"80:localhost:{WEB_PORT}", "serveo.net"],
        r"(https?://[a-zA-Z0-9-]+\.serveo\.net)",
        "Serveo.net"
    ): return

    # Attempt 2: Localhost.run
    if try_tunnel(
        ["ssh", "-o", "StrictHostKeyChecking=no", "-R", f"80:localhost:{WEB_PORT}", "nokey@localhost.run"],
        r"(https?://[a-zA-Z0-9-]+\.localhost\.run)",
        "Localhost.run"
    ): return

    # Attempt 3: Remote.moe
    if try_tunnel(
        ["ssh", "-o", "StrictHostKeyChecking=no", "-nNT", "-R", f"80:localhost:{WEB_PORT}", "remote.moe"],
        r"(https?://[a-zA-Z0-9-]+\.remote\.moe)",
        "Remote.moe"
    ): return

    logger.error("🚫 All tunnels failed.")
...
if __name__ == "__main__":
    ...
    # Attempt to start tunnel in a separate thread to not block bot startup
