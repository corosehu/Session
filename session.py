...
def send_welcome(message):
    ...
    if WEBAPP_URL:
        ...
        note = "_Running in Native Web App Mode (HTTPS)_"
    else:
        # Fallback to direct HTTP link
        public_ip = get_public_ip()
        public_url = f"http://{public_ip}:{WEB_PORT}"
        markup.add(telebot.types.InlineKeyboardButton("🌐 Open Dashboard (Direct)", url=public_url))
        note = "_Tunnel failed. Using Direct HTTP Link (Opens in Browser)_"
