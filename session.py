...
class Instagram:
    ...
    def _update_cookie_string(self, response=None):
        """Robustly update cookies from jar, headers, and raw response."""
        try:
            all_cookies = {}

            # 1. From Session CookieJar (Persistent)
            for cookie in self.session.cookies:
                all_cookies[cookie.name] = cookie.value

            # 2. From Response CookieJar (Immediate)
            if response:
                for cookie in response.cookies:
                    all_cookies[cookie.name] = cookie.value

            # 3. Aggressive Raw Header Parsing (Bypass Requests merging)
            # This handles cases where requests/urllib3 might drop or merge headers unexpectedly
            if response and hasattr(response, 'raw') and hasattr(response.raw, 'headers'):
                try:
                    # 'getlist' retrieves all headers with the same name as a list
                    getlist = getattr(response.raw.headers, 'getlist', None)
                    if getlist:
                        raw_cookie_lines = getlist('Set-Cookie')
                        if raw_cookie_lines:
                            logger.debug(f"Found {len(raw_cookie_lines)} raw Set-Cookie headers")
                            for line in raw_cookie_lines:
                                # Standard Cookie Format: name=value; Domain=...; ...
                                parts = line.split(';')
                                if parts:
                                    kv = parts[0].strip()
                                    if '=' in kv:
                     ...
                     logger.warning(f"Raw header parsing warning: {raw_e}")
            ...
            if "sessionid" in self.cookie_string:
                logger.info(f"✅ Captured sessionid for {self.username}")
            else:
                logger.warning(f"⚠️ Session ID MISSING. Found keys: {list(all_cookies.keys())}")
                if response:
                     logger.debug(f"Headers dump: {response.headers}")
...
async def api_login(
    ...
    logger.info(f"API Login Request: {username} (File: {filename})")
    try:
        ...
        return JSONResponse(status_code=500, content={"detail": f"System Error: {str(e)}"})
...
def save_session_custom(api_instance, username, password, gmail, gmail_password, filename):
    ...
    if "sessionid" not in api_instance.cookie_string:
        logger.error(f"Session ID still missing for {username} after update.")
        # Warn but save what we have? Or fail?
        # User explicitly complained about blank cookies. Let's warn in log.
        # Ideally we should fail, but let's persist what we have.
