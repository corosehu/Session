...
def send_welcome(message):
    ...
    markup.add(telebot.types.InlineKeyboardButton("🌐 Open Web Manager (Public)", url=public_url))
...
🔹 *Connection Info:*
1. *Public URL (Recommended for VPS):* `{public_url}`
2. *Local (Same Device):* `{local_url}`
3. *LAN (Same Network):* `{lan_url}`

⚠️ *Note:* If the Public URL doesn't work, ensure *Port 8000* is allowed in your server's firewall/security group.
