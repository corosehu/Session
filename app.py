...
# Key: username, Value: {'loader': Instaloader, 'password': str}
login_sessions = {}
...
<head>
    ...
    <style>
        ...
        .bg-grid-pattern {
            background-image: radial-gradient(circle at 50% 50%, #00F0FF 1px, transparent 1px);
        ...
        .btn-primary {
            @apply px-6 py-2 bg-primary-blue text-dark-bg font-bold hover:bg-cyan-300 transition-colors duration-300 tracking-wider uppercase text-sm;
        }
        .btn-gold {
            @apply px-6 py-2 bg-primary-gold text-dark-bg font-bold hover:bg-yellow-400 transition-colors duration-300 tracking-wider uppercase text-sm;
        }
        .input-field {
            @apply w-full bg-card-bg border border-gray-700 p-3 text-white focus:border-primary-blue focus:outline-none transition-colors duration-300 placeholder-gray-600;
        }
        .card {
            @apply bg-card-bg border border-gray-800 p-6 transition-all duration-300;
        }
        .card:hover {
            @apply border-primary-blue shadow-[0_0_15px_rgba(0,240,255,0.1)];
        }
    </style>
</head>
<body class="min-h-screen relative" x-data="appData()" x-init="fetchFiles()">
    ...
    <header className="w-full max-w-5xl flex justify-between items-center mb-12 border-b border-gray-800 pb-4">
        <div class="max-w-6xl mx-auto px-6 py-6 flex justify-between items-center border-b border-gray-800 mb-8">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-2xl md:text-3xl font-bold tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-primary-blue to-white orbitron">
                    NEXUS SESSION
                </h1>
            </div>
            <button x-show="view !== 'login'" @click="view = 'login'" class="text-gray-400 hover:text-white transition-colors text-sm uppercase tracking-wider">
                New Login
            </button>
        </div>
    </header>
    ...
    <main class="max-w-5xl mx-auto px-6 pb-20 flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <div x-show="view === 'login'" x-transition.opacity.duration.500ms class="w-full max-w-md">
            <div class="card shadow-[0_0_20px_rgba(0,240,255,0.1)]">
                ...
                <div class="mt-8 pt-6 border-t border-gray-800 text-center">
                    <button @click="view = 'dashboard'; fetchFiles()" class="text-gray-500 hover:text-primary-blue text-xs uppercase tracking-widest transition-colors">
                        View Existing Files
                    </button>
                </div>
            </div>
        ...
        <div x-show="view === 'gmail'" x-transition.opacity.duration.500ms class="w-full max-w-md" style="display: none;">
             <div class="card shadow-[0_0_20px_rgba(212,175,55,0.1)] border-primary-gold/30">
                <div class="flex flex-col items-center mb-8">
                    ...
                    <p class="text-gray-400 text-xs uppercase text-center mt-2 tracking-wide">
                        Provide Recovery Details
                    </p>
                </div>
        ...
        <div x-show="view === 'dashboard'" x-transition.opacity.duration.500ms class="w-full" style="display: none;">
            ...
            <div x-show="files.length === 0" class="text-center py-20 border border-dashed border-gray-800 rounded-lg">
                <p class="text-gray-500">No session files found.</p>
            </div>
    ...
    <script>
        function appData() {
            return {
                view: 'login', // login, gmail, dashboard
                username: '',
                password: '',
                gmail: '',
                gmailPass: '',
                ...
                async handleLogin() {
                    ...
                    try {
                        ...
                        if (data.status === 'pending' && data.requires_2fa) {
                            this.sessionUser = this.username;
                ...
                async handle2FASubmit() {
                    ...
                    try {
                        const res = await fetch('/api/2fa', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ username: this.sessionUser, code: this.twoFACode })
                        });
                        if (!res.ok) {
                            const data = await res.json();
                ...
                async handleGmailSubmit() {
                    ...
                    try {
                        const res = await fetch('/api/save', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ username: this.sessionUser, gmail: this.gmail, gmail_password: this.gmailPass })
                        });
                        if (!res.ok) {
                             const data = await res.json();
                             throw new Error(data.detail);
                        ...
                        // Reset
                        this.gmail = ''; this.gmailPass = ''; this.username = ''; this.password = '';
                        this.view = 'dashboard';
                        this.fetchFiles();
                    } catch (e) {
                        this.error = e.message;
                ...
                async handleEditSubmit() {
                    ...
                    try {
                        const res = await fetch('/api/edit', {
                            ...
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
                                filename: this.editFile,
                                gmail: this.editGmail,
                                gmail_password: this.editGmailPass
                            })
                        });
